...
client = MongoClient('mongodb+srv://mugil1206:mugilan@cluster0.zhm3u.mongodb.net/')
db = client['Data_Profiling']
...
def register(request):
    if request.method == 'POST':
        ...
        if not re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{8,}$', password):
            return render(request, 'register.html', {'error': 'Password must be at least 8 characters long, contain uppercase and lowercase letters, and include symbols.'})
        ...
        otp_collection.insert_one({'email': email, 'otp': otp, 'created_at': timezone.now()})
...
def verify_otp(request, email):
    if request.method == 'POST':
        ...
        if otp_record and otp_record['otp'] == otp:
            ...
            otp_created_at = make_aware(otp_record['created_at'])
            current_time = timezone.now()

            if (current_time - otp_created_at).total_seconds() < 600:
                # OTP is valid
...
def login(request):
    if request.method == 'POST':
        ...
        if user and check_password(password, user['password']):
            ...
            return render(request, 'login.html', {'error': 'Invalid credentials. If you do not have an account, please register.'})
